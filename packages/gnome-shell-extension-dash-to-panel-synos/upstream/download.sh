...
for SUITE in "${!GNOME_TARGETS[@]}"; do
    ...
    echo "[$SUITE] Prioritizing Dash to Panel in the SynOS session..."
...
metadata["session-modes"] = ["user", "synos"]
...
PY
    ...
    echo "[$SUITE] Applying SynOS panel layout to $DEPLOY_DIR/panelPositions.js..."

    sed -i '/export const defaults = \[/,/^\]$/c\
// SynOS custom default panel layout\
