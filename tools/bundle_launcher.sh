...
say "To install it:"
if [ -n "$macos" ]; then
    say "  USB stick:  write the ISO with balenaEtcher (https://etcher.balena.io), or"
    say "              sudo dd if='$iso' of=/dev/diskN bs=4m status=progress"
    say "              The ISO must be written whole, byte for byte. A tool that copies"
    say "              its *files* onto the stick instead produces one that boots and then"
    say "              fails with squashfs read or decompression errors."
    ...
    say "  USB stick:  write the ISO with balenaEtcher, Fedora Media Writer,"
    say "              GNOME Disks (\"Restore Disk Image\"), or"
    say "              sudo dd if='$iso' of=/dev/sdX bs=4M status=progress oflag=sync conv=fsync"
    say "              Not Ubuntu'\''s \"Startup Disk Creator\", not Rufus in ISO mode, not"
    say "              UNetbootin, and not Ventoy: those copy or rearrange the ISO'\''s files"
    say "              instead of writing it whole, which this image cannot boot from - it"
    say "              starts and then fails with squashfs read or zstd decompression"
    say "              errors. If that happens, boot the ISO'\''s own \"Check installation"
    say "              media for defects\" entry to confirm the stick, then write it again."
