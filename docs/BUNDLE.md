...
**A relative path is resolved to an absolute one before anything uses it**
— `--storage storage` (and the older `--container-root`, and
`SYNOS_CONTAINER_ROOT`) is read from the directory the launcher was run in,
made absolute right there, and used absolute everywhere after: the runtime
arguments, the free-space and overlay checks, the remembered value and the
line that reports where the storage is. This matters because podman is not
necessarily started from that same directory by the time it reads the
value — under `sudo`, or partway through building the engine image locally
(which changes into the engine source first) — so a relative path handed to
it directly would be interpreted by whatever directory podman finds itself
in, not the one the person was sitting in. A path containing a space
survives every one of these hops unchanged; the runtime is always invoked
with real, individually quoted arguments, never a flag and a path rebuilt
into one string and split back apart.

**A storage location inside a directory this build copies, archives or
hashes whole is refused**, naming both paths: the engine source (podman
`build`'s own `COPY . /opt/synos` walks everything under it — storage
written there mid-build has broken a real build with pipe and permission
errors partway through that `COPY`) and the output directory `dist/` (the
evidence files next to the ISO — the SBOM, the checksums — are generated by
reading it whole). This check is a rule, not a reflex: it looks at where
the storage would actually sit, not at what an `.containerignore` happens
to exclude for unrelated reasons; the documented default
(`.build/container-storage`) is unaffected, since it is a sibling of both,
never inside either.
